...
import jp.osaka.cherry.stamp.databinding.ActivityBlueBinding;
 ...
 * メインアクティビティ
 */
public class BlueActivity extends AppCompatActivity implements
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // レイアウト設定
        ActivityBlueBinding binding = DataBindingUtil.setContentView(this, R.layout.activity_blue);
        ...
        if (bar != null) {
            bar.setTitle(R.string.blue_stamp);
        }
        ...
        // 登録
        mState.setId(R.id.blue);
        mState.registerCallacks(this);

        if (!mRedos.isEmpty()) {
            mPref.edit().putInt(EXTRA_CONTENT, R.id.blue).apply();
        }
    }
     ...
     */
    @Override
    public void onUpdated(Object object, final List<SimpleStamp> stampList) {
        mHandler.post(new Runnable() {
            ...
            public void run() {
                try {
                    boolean result;
                    result = mDataSet.isEmpty() || mDataSet.size() != stampList.size() || !mDataSet.equals(stampList);
                    if (result) {
                        // レジューム状態の確認
                        if (mState.isResumed()) {
                            return;
    ...
    private void updateTitle(ArrayList<SimpleStamp> collection) {
        ...
        if (collection.isEmpty()) {
            sb.append(this.getString(R.string.blue_stamp));
        } else {
            sb.append(this.getString(R.string.blue_stamp)).append("  ").append(collection.size());
        }
